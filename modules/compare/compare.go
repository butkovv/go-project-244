...
func Compare(first, second map[string]any) map[string]DiffEntry {
	...
	for _, k := range keys {
		...
		v2, ok2 := second[k]
		if !ok1 && ok2 {
			res[k] = DiffEntry{Status: StatusAdded, FirstValue: nil, SecondValue: v2, IsNested: false, Diff: nil}
		}
		if !ok2 && ok1 {
			res[k] = DiffEntry{Status: StatusRemoved, FirstValue: v1, SecondValue: nil, IsNested: false, Diff: nil}
		}
		if ok1 && ok2 && IsMap(v1) && IsMap(v2) {
			m1, ok1 := v1.(map[string]any)
			m2, ok2 := v2.(map[string]any)
			if reflect.DeepEqual(m1, m2) {
				res[k] = DiffEntry{Status: StatusUnchanged, FirstValue: v1, SecondValue: v2, IsNested: false, Diff: nil}
			} else if ok1 && ok2 {
				res[k] = DiffEntry{Status: StatusChanged, FirstValue: v1, SecondValue: v2, IsNested: true, Diff: Compare(m1, m2)}
			}
		}
		if ok1 && ok2 && ((IsMap(v1) && !IsMap(v2)) || (!IsMap(v1) && IsMap(v2)) || (!IsMap(v1) && !IsMap(v2) && v1 != v2)) {
			res[k] = DiffEntry{Status: StatusChanged, FirstValue: v1, SecondValue: v2, IsNested: false, Diff: nil}
		}
